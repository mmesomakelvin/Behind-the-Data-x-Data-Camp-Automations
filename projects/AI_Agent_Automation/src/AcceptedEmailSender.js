...
function sendAcceptedEmails() {
  withScriptLock_(function () {
    ...
    const reviewColumns = getReviewColumnIndexes_(sourceSheet);
    const sourceColumns = getSourceColumnIndexes_(sourceSheet);
    const emailStatusInfo = ensureAcceptedEmailStatusColumns_(sourceSheet);
    const lastRow = sourceSheet.getLastRow();

    if (lastRow < 2) {
      logAndToast_("No source rows found for accepted email sending.");
      return;
    }
    ...
    for (let rowNumber = 2; rowNumber <= lastRow; rowNumber++) {
      ...
      const email = normalizeEmail_(row[reviewColumns.emailIndex]);
      const statusInfo = getReviewStatusInfo_(row[reviewColumns.statusIndex]);
      const currentSendStatus = String(row[emailStatusInfo.statusIndex] || "").trim().toLowerCase();
      const fullName = String(row[sourceColumns.fullNameIndex] || "").trim();
      ...
      if (!email) {
        sourceSheet.getRange(rowNumber, emailStatusInfo.statusIndex + 1).setValue("Skipped - No Email");
        skipped++;
        continue;
      }
      ...
      try {
        ...
        sourceSheet.getRange(rowNumber, emailStatusInfo.statusIndex + 1).setValue("Sent");
        sourceSheet.getRange(rowNumber, emailStatusInfo.errorIndex + 1).setValue("");
        sourceSheet.getRange(rowNumber, emailStatusInfo.sentAtIndex + 1).setValue(new Date());
        sent++;

        Utilities.sleep(200);
      } catch (err) {
        sourceSheet.getRange(rowNumber, emailStatusInfo.statusIndex + 1).setValue("Failed");
        sourceSheet.getRange(rowNumber, emailStatusInfo.errorIndex + 1).setValue(String(err));
        failed++;
      }
    }
